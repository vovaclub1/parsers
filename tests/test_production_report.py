...
def test_production_report_combines_execution_tca_and_reconciliation(tmp_path):
    ...
    assert report["reconciliation"]["closed"] == 1
    assert report["reconciliation"]["realized_exec_pnl"] == 9
    assert report["reconciliation"]["recorded_fees"] == .2
    assert report["reconciliation"]["pnl_after_recorded_fees"] == 8.8
